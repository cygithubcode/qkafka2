/q tick.q SRC [DST] [-p 5010] [-o h]
system"l tick/",(src:first .z.x,enlist"sym"),".q";

if[not system"p";system"p 5010"];

\l tick/u.q
\d .u

\l ini_producer.q


// use this ld to turn on logfile
//ld:{if[not type key L::`$(-10_string L),string x;.[L;();:;()]];i::j::-11!(-2;L);if[0<=type i;-2 (string L)," is a corrupt log. Truncate to length ",(string last i)," and restart";exit 1];hopen L};

// use this ld without logfile
ld:{i::j::0;0};


tick:{init[];if[not min(`time`sym~2#key flip value@)each t;'`timesym];@[;`sym;`g#]each t;d::.z.D;if[l::count y;L::`$":",y,"/",x,10#".";l::ld d]};

endofday:{end d;d+:1;if[l;hclose l;l::0(`.u.ld;d)]};
ts:{if[d<x;if[d<x-1;system"t 0";'"more than one day?"];endofday[]]};

if[system"t"; show "means_start_with_a_timer";
 .z.ts:{pub'[t;value each t];@[`.;t;@[;`sym;`g#]0#];i::j;ts .z.D};
 upd:{[t;x]
 if[not -12=type first first x;if[d<"d"$a:.z.P;.z.ts[]];a:"p"$a;x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
 t insert x;
	// publish to kafka
	show x; .kfk.Pub[test_topic;.kfk.PARTITION_UA;string enlist (`upd;t;x);"kxtick"];
	
	if[l;
		// l enlist (`upd;t;x);
		// .kfk.Pub[test_topic;.kfk.PARTITION_UA;string .z.p;"kxtick "];
		j+:1];
	
	}];

if[not system"t";system"t 1000";
 .z.ts:{ts .z.D};
 upd:{[t;x]ts"d"$a:.z.P;
 if[not -12=type first first x;a:"p"$a;x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
 f:key flip value t;
 
	pub[t;$[0>type first x;enlist f!x;flip f!x]];
	show x; 
	//.kfk.Pub[test_topic;.kfk.PARTITION_UA;-8! enlist (`upd;t;x);"kxtick"];
	.kfk.Pub[test_topic;.kfk.PARTITION_UA;-8! (`upd;t;x);"kxtick"];
	
	// if[l;l enlist (`upd;t;x);i+:1];
	
	}];

\d .
.u.tick[src;.z.x 1];

\
 globals used
 .u.w - dictionary of tables->(handle;syms)
 .u.i - msg count in log file
 .u.j - total msg count (log file plus those held in buffer)
 .u.t - table names
 .u.L - tp log filename, e.g. `:./sym2008.09.11
 .u.l - handle to tp log file
 .u.d - date

/test
>q tick.q
>q tick/ssl.q

/run
>q tick.q sym  .  -p 5010   /tick
>q tick/r.q :5010 -p 5011   /rdb
>q sym            -p 5012   /hdb
>q tick/ssl.q sym :5010     /feed
